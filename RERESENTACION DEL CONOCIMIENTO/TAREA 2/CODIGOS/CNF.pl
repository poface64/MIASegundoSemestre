...
% Caso 1 nnf: si FI es una literal, devuelve esa literal
nnf(X, X) :- atom(X).  % Lo devuelve tal cual.

% Caso 2 nnf: Si ~~fi1, devuelve fi1
nnf(n(n(X)), NNF) :- nnf(X, NNF).  % ¬¬ϕ1 => ϕ1

% Caso 3 nnf: FI1 Y FI2 regresa nnf(FI1) Y nnf(FI2)
nnf(X ^ Y, NNF) :- 
    nnf(X, NNF_X),
    nnf(Y, NNF_Y),
    NNF = NNF_X ^ NNF_Y.

% Caso 4 nnf: FI1 O FI2 regresa nnf(FI1) O nnf(FI2)
nnf(X v Y, NNF) :- 
    nnf(X, NNF_X),
    nnf(Y, NNF_Y),
    NNF = NNF_X v NNF_Y.

% Caso 5 nnf: neg(FI1 Y FI2) regresa neg(FI1) O neg(FI2)
nnf(n(X ^ Y), NNF) :- 
    nnf(n(X), NNF_X),
    nnf(n(Y), NNF_Y),
    NNF = NNF_X v NNF_Y.

% Caso 6 nnf: neg(FI1 O FI2) regresa neg(FI1) Y neg(FI2)
nnf(n(X v Y), NNF) :- 
    nnf(n(X), NNF_X),
    nnf(n(Y), NNF_Y),
    NNF = NNF_X ^ NNF_Y.

% Caso 7 nnf: FI1 => FI2 se convierte en neg(FI1) O FI2
nnf(X => Y, NNF) :-
    nnf(n(X), NNF_X),  % Negamos X
