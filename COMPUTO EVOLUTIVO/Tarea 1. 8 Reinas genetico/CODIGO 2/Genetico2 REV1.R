...
padreselect = function(aptitudes,indices,PC ){
...
reempplazo = function(megamix,pobsize){
  ...
  # Ordenar los posibles candidatos
  candidatos = candidatos[order(candidatos[,1],decreasing = F),]
  # Seleccionar unicamente a los primeros desde 1:pobsize despues de ordenarlos
  # de mejor a peor
  nuevapob = candidatos[1:pobsize,]
  # Devolver los seleccionados
  return(nuevapob)
}
...
table(X = factor(megamix[[npobla[10,2]]][,1],levels = 1:8) ,
      Y = factor(megamix[[npobla[10,2]]][,2],levels = 1:8))
