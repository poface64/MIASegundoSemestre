...
evaluarpob = function(poblacion, data, umbral,alpha = 1000){
  ...
  for (i in seq_along(poblacion)) {
    ...
    # Calcular su  mdl #
    mdl_score = mdl(adj, data)
    # Calcular su penalización
    ciclo_penal = penalizacion_ciclos(adj)
    # Calcular el fitness final
    fitness = mdl_score + alpha * ciclo_penal
